...
public class BestTimeToBuySell2 {
	...
	//	System.out.println("Ans : " + maxProfit(arr));
		...
		int[] arr2 = {1,2,3,4,5};
	//	System.out.println("Ans : " + maxProfit(arr2));
		
		int[] arr3 = {7,6,4,3,1};
		System.out.println("Ans : " + maxProfit(arr3));
	...
	public static int maxProfit(int[] prices) {
		...
		for(int i= 1; i < prices.length; i++) {
			int temp = transaction(buy, prices[i]);
		//	System.out.println(temp);
			if( temp!= 0 && i+1 < prices.length && i> i+1 ) {	
				buy = prices[i+1];
			}else {
				buy = prices[i];
			}
	...
	public static int transaction(int buy, int sell) {
	//	System.out.println("buy " +  buy + " sell " +  sell);
		if(buy> sell)
		return 0;
		else return Math.abs(sell- buy);
	}
